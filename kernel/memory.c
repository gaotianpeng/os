...
void* malloc_page(enum pool_flags pf, uint32_t pg_cnt) {
    ...
    /*
        ...
        3 通过page_table_addr 将以上得到的虚拟地址和物理地址在页表中完成映射
    */
...
// 安装1页大小的vaddr, 专门针对fork时虚拟地址位置无须操作的情况
...
static struct mem_block* arena2block(struct arena* a, uint32_t idx) {
    return (struct mem_block*)((uint32_t)a + 
                sizeof(struct arena) + idx * a->desc->block_size);
}
...
void* sys_malloc(uint32_t size) {
    ...
    if (size > 1024) {
        ...
        /* 
            若 mem_block_desc 的 free_list 中已经没有可用的mem_block，
...
/*
    ...
    2）在页表中去掉虚拟地址的映射，原理是将虚拟地址对应pte的P位置0（page_table_pte_remove）
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    // page_table_size 用来记录页目录表和页表占用的字节大小，总大小 = 页目录表大小 + 页表大小
    // 页表大小 = 1页的页目录表 + 第0和第768个页目录项指向同一个页表 + 第769~1022个页目录项共指向254个页表，共256个页
    uint32_t page_table_size = PG_SIZE * 256;
    // used_mem 记录当前已使用的内存字节数 = page_table_size + 低端0x100000字节(1MB)内存
    uint32_t used_mem = page_table_size + 0x100000;	        // 0x100000为低端1M内存
    // free_mem 记录目前可用的内存字节数
