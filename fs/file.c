...
// 把buf中count个字节写入file, 成功则返回写入的字节数, 失败则返回-1
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
    if ((file->fd_inode->i_size + count) > (BLOCK_SIZE * 140)) {
        printk("exceed max fild_size 71680, write file failed\n");
        return -1;
    }
    ...
    // 开始将文件所有块地址收集到all_blocks, 后面都统一在all_blocks中获取写入扇区地址
    if (add_blocks == 0) {
        // 同一扇区内写入数据，不涉及到分配新扇区
...
int32_t file_read(struct file* file, void* buf, uint32_t count) {
    ...
    uint32_t* all_blocks = (uint32_t*)sys_malloc(BLOCK_SIZE + 48); // 用来记录文件所有地块地址
    if (all_blocks == NULL) {
